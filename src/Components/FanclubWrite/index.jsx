...
export default class FanclubWrite extends Component {
  ...
  @observable verifiedYoutubeLink = '';
  ...
  @action validation = () => {
    ...
    if (this.currentType === 'video') {
      if (this.postVideoSrc === '' && !this.youtubeFlag) {
        ...
        fanclubStore.updateMsgWindowOpen('true');
        return false;
      } else if (this.verifiedYoutubeLink === '' && this.youtubeFlag) {
        fanclubStore.updateMsgWindow({
  ...
  @action registerToServer = async () => {
    ...
    switch (this.currentType) {
      ...
      case 'image':
        ...
        console.log('업로드한 이미지', registerObj.image);
        break;
      case 'video':
        registerObj.type = '5';
        if (self.youtubeFlag) {
    ...
    const result = await fanclubStore.registerPost(registerObj);
  ...
  render() {
    ...
    return (
      <div className="fanclub-write-container">
        ...
        <ViewPageHedaer title={'글쓰기'} searchFlag={true} settingFlag={true} />
        ...
        <div className="tit-write-section box-type">
          <div className="inp-wrap">
            <input type="text" className="inp-text" name="postTitle" placeholder="제목을 입력해 주세요." onChange={this.onChange} />
          </div>
        </div>
        ...
        {this.currentType === 'image' && (
          <div className="media-wrapper">
            <div className="media-upload">
              <label className="btn-type1 bg-color1 alter-type1">
                이미지 등록하기
        ...
        <div className="content-write-section box-type">
          <div className="inp-wrap">
            <textarea className="inp-textarea" wrap="hard" name="postContent" placeholder="내용을 입력해 주세요." cols="30" onChange={this.onChange}></textarea>
          </div>
